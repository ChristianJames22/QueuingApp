...
import com.example.appque.databinding.ActivityWindow3Binding
...
class Window3Activity : AppCompatActivity() {
    ...
    private lateinit var binding: ActivityWindow3Binding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityWindow3Binding.inflate(layoutInflater)
        ...
        binding.appointmentsRecyclerView.apply {
            layoutManager = LinearLayoutManager(this@Window3Activity)
        ...
        database.child("window3Queue").child("appointments").addValueEventListener(object : ValueEventListener {
            ...
            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@Window3Activity, "Error: ${error.message}", Toast.LENGTH_SHORT).show()
            }
        })
    ...
    private fun fetchWindow3Status() {
        val windowId = "window3"
        database.child("window3Status").child(windowId).addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
            ...
            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@Window3Activity, "Failed to fetch status: ${error.message}", Toast.LENGTH_SHORT).show()
            }
        })
    }
    ...
    private fun updateFirebaseStatus(isOnBreak: Boolean? = null, isOffline: Boolean? = null) {
        val windowId = "window3"
        isOnBreak?.let { database.child("window3Status").child(windowId).child("onBreak").setValue(it) }
        isOffline?.let { database.child("window3Status").child(windowId).child("offline").setValue(it) }
    }
    ...
    private fun toggleOnBreak() {
        ...
        Toast.makeText(this, if (isOnBreak) "Window is now On Break." else "Welcome back!", Toast.LENGTH_SHORT).show()
        updateUI()
    }
    ...
    private fun toggleOffline() {
        ...
        Toast.makeText(this, if (isOffline) "Window is now Offline." else "You are now Online.", Toast.LENGTH_SHORT).show()
        updateUI()
    }
    ...
    private fun resetQueue() {
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Yes") { _, _ ->
                database.child("window3Queue").child("appointments").removeValue()
                database.child("window3Queue").child("currentQueueNumber").setValue(0)
    ...
    private fun moveToNextAppointment() {
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Yes") { _, _ ->
                ...
                database.child("window3Queue").child("appointments").get().addOnSuccessListener { snapshot ->
                    val appointments = snapshot.children.toList()
                    if (appointments.isNotEmpty()) {
                        val firstKey = appointments.first().key
                        firstKey?.let {
                            database.child("window3Queue").child("appointments").child(it).removeValue()
