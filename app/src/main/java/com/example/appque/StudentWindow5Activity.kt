...
class StudentWindow5Activity : AppCompatActivity() {
    ...
    private fun fetchCurrentQueueNumber() {
        database.child("window5Queue").child("currentQueueNumber")
    ...
    private fun setupRealTimeUserActiveAppointmentListener() {
        database.child("window5Queue").child("appointments")
            .addValueEventListener(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
    ...
    private fun observeWindow5Status() {
        val cashierId = "window5" // ID for the cashier node
        database.child("window5Status").child(cashierId).addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
    ...
    private fun addAppointment(selectedAppointment: String) {
        ...
        database.child("window5Queue").child("appointments").push().setValue(appointment)
            .addOnSuccessListener {
                database.child("window5Queue").child("currentQueueNumber").setValue(currentQueueNumber);
