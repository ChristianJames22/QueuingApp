...
class Window6Activity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.appointmentsRecyclerView.apply {
            layoutManager = LinearLayoutManager(this@Window6Activity)
        ...
        database.child("window6Queue").child("appointments").addValueEventListener(object : ValueEventListener {
            ...
            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@Window6Activity, "Error: ${error.message}", Toast.LENGTH_SHORT).show()
            }
        })
    ...
    private fun fetchWindow6Status() {
        val windowId = "window6"
        database.child("window6Status").child(windowId).addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
            ...
            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@Window6Activity, "Failed to fetch window status: ${error.message}", Toast.LENGTH_SHORT).show()
            }
        })
    }
    ...
    private fun updateFirebaseStatus(isOnBreak: Boolean? = null, isOffline: Boolean? = null) {
        val windowId = "window6"
        isOnBreak?.let { database.child("window6Status").child(windowId).child("onBreak").setValue(it) }
        isOffline?.let { database.child("window6Status").child(windowId).child("offline").setValue(it) }
    }
    ...
    private fun resetQueue() {
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Yes") { _, _ ->
                database.child("window6Queue").child("appointments").removeValue()
                database.child("window6Queue").child("currentQueueNumber").setValue(0)
    ...
    private fun moveToNextAppointment() {
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Yes") { _, _ ->
                ...
                database.child("window6Queue").child("appointments").get().addOnSuccessListener { snapshot ->
                    val appointments = snapshot.children.toList()
                    if (appointments.isNotEmpty()) {
                        val firstKey = appointments.first().key
                        firstKey?.let {
                            database.child("window6Queue").child("appointments").child(it).removeValue()
