...
class Window6Activity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        database.child("window6").child("appointments").addValueEventListener(object : ValueEventListener {
    ...
    private fun resetQueue() {
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Yes") { _, _ ->
                database.child("window6").child("appointments").removeValue()
                database.child("window6").child("currentQueueNumber").setValue(0)
    ...
    private fun moveToNextAppointment() {
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Yes") { _, _ ->
                ...
                database.child("window6").child("appointments").get().addOnSuccessListener { snapshot ->
                    val appointments = snapshot.children.toList()
                    if (appointments.isNotEmpty()) {
                        val firstKey = appointments.first().key
                        firstKey?.let {
                            database.child("window6").child("appointments").child(it).removeValue()
