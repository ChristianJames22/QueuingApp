...
import com.example.appque.databinding.ActivityWindow4Binding;
...
class Window4Activity : AppCompatActivity() {
    ...
    private lateinit var binding: ActivityWindow4Binding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityWindow4Binding.inflate(layoutInflater)
        ...
        binding.appointmentsRecyclerView.apply {
            layoutManager = LinearLayoutManager(this@Window4Activity)
        ...
        database.child("window4Queue").child("appointments").addValueEventListener(object : ValueEventListener {
            ...
            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@Window4Activity, "Error: ${error.message}", Toast.LENGTH_SHORT).show()
            }
        })
    ...
    private fun fetchWindow4Status() {
        val windowId = "window4"
        database.child("window4Status").child(windowId).addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
            ...
            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@Window4Activity, "Failed to fetch window status: ${error.message}", Toast.LENGTH_SHORT).show()
            }
        })
    }
    ...
    private fun updateFirebaseStatus(isOnBreak: Boolean? = null, isOffline: Boolean? = null) {
        val windowId = "window4"
        isOnBreak?.let { database.child("window4Status").child(windowId).child("onBreak").setValue(it) }
        isOffline?.let { database.child("window4Status").child(windowId).child("offline").setValue(it) }
    }
    ...
    private fun confirmToggleOnBreak() {
        val message = if (isOnBreak) "turn off On Break?" else "set the window to On Break?"
    ...
    private fun confirmToggleOffline() {
        val message = if (isOffline) "turn off Offline?" else "set the window to Offline?"
        AlertDialog.Builder(this)
    ...
    private fun toggleOnBreak() {
        ...
        Toast.makeText(this, if (isOnBreak) "Window 4 is now On Break." else "Welcome back!", Toast.LENGTH_SHORT).show()
        updateUI()
    }
    ...
    private fun toggleOffline() {
        ...
        Toast.makeText(this, if (isOffline) "Window 4 is now Offline." else "You are now Online.", Toast.LENGTH_SHORT).show()
        updateUI()
    }
    ...
    private fun resetQueue() {
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Yes") { _, _ ->
                database.child("window4Queue").child("appointments").removeValue()
                database.child("window4Queue").child("currentQueueNumber").setValue(0)
    ...
    private fun moveToNextAppointment() {
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Yes") { _, _ ->
                ...
                database.child("window4Queue").child("appointments").get().addOnSuccessListener { snapshot ->
                    val appointments = snapshot.children.toList()
                    if (appointments.isNotEmpty()) {
                        val firstKey = appointments.first().key
                        firstKey?.let {
                            database.child("window4Queue").child("appointments").child(it).removeValue()
