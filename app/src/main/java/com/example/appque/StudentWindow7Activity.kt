...
class StudentWindow7Activity : AppCompatActivity() {
    ...
    private fun fetchCurrentQueueNumber() {
        database.child("window7Queue").child("currentQueueNumber")
    ...
    private fun setupRealTimeUserActiveAppointmentListener() {
        database.child("window7Queue").child("appointments")
            .addValueEventListener(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
    ...
    private fun observeWindow7Status() {
        val cashierId = "window7" // ID for the cashier node
        database.child("window7Status").child(cashierId).addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
    ...
    private fun addAppointment(selectedAppointment: String) {
        ...
        database.child("window7Queue").child("appointments").push().setValue(appointment)
            .addOnSuccessListener {
                database.child("window7Queue").child("currentQueueNumber").setValue(currentQueueNumber);
