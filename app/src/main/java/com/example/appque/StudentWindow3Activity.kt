...
class StudentWindow3Activity : AppCompatActivity() {
    ...
    private fun fetchCurrentQueueNumber() {
        database.child("window3Queue").child("currentQueueNumber")
    ...
    private fun setupRealTimeUserActiveAppointmentListener() {
        database.child("window3Queue").child("appointments")
            .addValueEventListener(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
    ...
    private fun observeWindow3Status() {
        val cashierId = "window3" // ID for the cashier node
        database.child("window3Status").child(cashierId).addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
    ...
    private fun addAppointment(selectedAppointment: String) {
        ...
        database.child("window3Queue").child("appointments").push().setValue(appointment)
            .addOnSuccessListener {
                database.child("window3Queue").child("currentQueueNumber").setValue(currentQueueNumber);
