...
class CashierActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        database.child("window1Queue").child("appointments").addValueEventListener(object : ValueEventListener {
    ...
    private fun fetchCashierStatus() {
        val cashierId = "cashier1"
        database.child("cashierStatus").child(cashierId).addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
            ...
            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@CashierActivity, "Failed to fetch cashier status: ${error.message}", Toast.LENGTH_SHORT).show()
            }
        })
    }
    ...
    private fun updateFirebaseStatus(isOnBreak: Boolean? = null, isOffline: Boolean? = null) {
        val cashierId = "cashier1"
        isOnBreak?.let { database.child("cashierStatus").child(cashierId).child("onBreak").setValue(it) }
        isOffline?.let { database.child("cashierStatus").child(cashierId).child("offline").setValue(it) }
    }
    ...
    private fun toggleOnBreak() {
        ...
        Toast.makeText(this, if (isOnBreak) "Cashier is now On Break." else "Welcome back!", Toast.LENGTH_SHORT).show()
        updateUI()
    }
    ...
    private fun toggleOffline() {
        ...
        Toast.makeText(this, if (isOffline) "Cashier is now Offline." else "You are now Online.", Toast.LENGTH_SHORT).show()
        updateUI()
    }
            ...
            .setPositiveButton("Yes") { _, _ ->
                ...
                database.child("window1Queue").child("appointments").get().addOnSuccessListener { snapshot ->
                    ...
                    if (appointments.isNotEmpty()) {
                        ...
                        firstKey?.let {
                            database.child("window1Queue").child("appointments").child(it).removeValue()
