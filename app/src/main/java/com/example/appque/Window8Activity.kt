...
class Window8Activity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.appointmentsRecyclerView.apply {
            layoutManager = LinearLayoutManager(this@Window8Activity)
        ...
        database.child("window8Queue").child("appointments").addValueEventListener(object : ValueEventListener {
    ...
    private fun fetchWindow8Status() {
        val window8Id = "window8"
        database.child("window8Status").child(window8Id).addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
            ...
            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@Window8Activity, "Failed to fetch status: ${error.message}", Toast.LENGTH_SHORT).show()
            }
        })
    }
    ...
    private fun updateFirebaseStatus(isOnBreak: Boolean? = null, isOffline: Boolean? = null) {
        val window8Id = "window8"
        isOnBreak?.let { database.child("window8Status").child(window8Id).child("onBreak").setValue(it) }
        isOffline?.let { database.child("window8Status").child(window8Id).child("offline").setValue(it) }
    }
    ...
    private fun toggleOnBreak() {
        ...
        Toast.makeText(this, if (isOnBreak) "Now On Break." else "Welcome back!", Toast.LENGTH_SHORT).show()
        updateUI()
    }
    ...
    private fun toggleOffline() {
        ...
        Toast.makeText(this, if (isOffline) "Now Offline." else "You are now Online.", Toast.LENGTH_SHORT).show()
        updateUI()
    }
    ...
    private fun resetQueue() {
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Yes") { _, _ ->
                database.child("window8Queue").child("appointments").removeValue()
                database.child("window8Queue").child("currentQueueNumber").setValue(0)
    ...
    private fun moveToNextAppointment() {
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Yes") { _, _ ->
                ...
                database.child("window8Queue").child("appointments").get().addOnSuccessListener { snapshot ->
                    val appointments = snapshot.children.toList()
                    if (appointments.isNotEmpty()) {
                        val firstKey = appointments.first().key
                        firstKey?.let {
                            database.child("window8Queue").child("appointments").child(it).removeValue()
