...
class StudentWindow4Activity : AppCompatActivity() {
    ...
    private fun fetchCurrentQueueNumber() {
        database.child("window4Queue").child("currentQueueNumber")
    ...
    private fun setupRealTimeUserActiveAppointmentListener() {
        database.child("window4Queue").child("appointments")
            .addValueEventListener(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
    ...
    private fun observeWindow4Status() {
        val cashierId = "window4" // ID for the cashier node
        database.child("window4Status").child(cashierId).addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
    ...
    private fun addAppointment(selectedAppointment: String) {
        ...
        database.child("window4Queue").child("appointments").push().setValue(appointment)
            .addOnSuccessListener {
                database.child("window4Queue").child("currentQueueNumber").setValue(currentQueueNumber);
